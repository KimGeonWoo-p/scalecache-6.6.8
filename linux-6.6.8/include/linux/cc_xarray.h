...
static inline void cc_xas_advance(struct cc_xa_state *xas, unsigned long index)
{
	unsigned char shift = cc_xas_is_node(xas) ? xas->xa_node->shift : 0;

	xas->xa_index = index;
	xas->xa_offset = (index >> shift) & CC_XA_CHUNK_MASK;
}
