 ...
 */
int cc_folio_migrate_mapping(struct address_space *mapping,
		...
		struct folio *newfolio, struct folio *folio, int extra_count)
{
	...
	if (cc_xas_is_ccxarray(&xas))
		return cc_folio_migrate_mapping(mapping, newfolio, folio, extra_count);
...
EXPORT_SYMBOL(cc_folio_migrate_mapping);

/*
 * The expected number of remaining references is the same as that
 * of folio_migrate_mapping().
 */
int cc_migrate_huge_page_move_mapping(struct address_space *mapping,
				   ...
				   struct folio *dst, struct folio *src)
{
	...
	if (cc_xas_is_ccxarray(&xas))
		return cc_migrate_huge_page_move_mapping(mapping, dst, src);
...
int cc_migrate_huge_page_move_mapping(struct address_space *mapping,
				   struct folio *dst, struct folio *src)
{
	CC_XA_STATE(xas, CC_XARRAY(&mapping->i_pages), folio_index(src));
	int expected_count;
