...
EXPORT_TRACEPOINT_SYMBOL_GPL(bdi_dirty_ratelimit);
...
EXPORT_SYMBOL_GPL(totalreserve_pages);
...
EXPORT_SYMBOL_GPL(ratelimit_pages);
...
EXPORT_SYMBOL_GPL(dirty_background_bytes);
...
EXPORT_SYMBOL_GPL(vm_dirty_bytes);
...
EXPORT_SYMBOL_GPL(bdi_min_ratio);
...
EXPORT_SYMBOL_GPL(bdp_ratelimits);
			     ...
			     pgoff_t start, pgoff_t end)
{
	...
	if (cc_xas_is_ccxarray(&xas))
		return cc_tag_pages_for_writeback(mapping, start, end);
...
EXPORT_SYMBOL(cc_tag_pages_for_writeback);
