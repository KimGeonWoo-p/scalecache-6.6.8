...
extern int bdi_set_max_ratio(struct backing_dev_info *bdi, unsigned int max_ratio);
/* ziggy
...
extern long pxt4_wb_do_writeback_modified(struct bdi_writeback *wb,
 ...
 */
static int pxt4_balance_dirty_pages(struct bdi_writeback *wb,
			       unsigned long pages_dirtied, unsigned int flags)
{
 ...
 */
int pxt4_balance_dirty_pages_ratelimited_flags(struct address_space *mapping,
					unsigned int flags)
{
	...
	if (unlikely(current->nr_dirtied >= ratelimit))
		ret = pxt4_balance_dirty_pages(wb, current->nr_dirtied, flags);

	wb_put(wb);
	return ret;
}
EXPORT_SYMBOL_GPL(pxt4_balance_dirty_pages_ratelimited_flags);
 ...
 */
void pxt4_balance_dirty_pages_ratelimited(struct address_space *mapping)
{
	pxt4_balance_dirty_pages_ratelimited_flags(mapping, 0);
}
EXPORT_SYMBOL(pxt4_balance_dirty_pages_ratelimited);
...
//bool __cc_folio_start_writeback(struct folio *folio, bool keep_write);
...
//			folio_memcg_unlock(folio);
//			return __cc_folio_start_writeback(folio, keep_write);
//		}
//
...
//	folio_memcg_lock(folio);
//	if (mapping && mapping_use_writeback_tags(mapping)) {
//		CC_XA_STATE(xas, CC_XARRAY(&mapping->i_pages), folio_index(folio));
//		struct inode *inode = mapping->host;
//		struct backing_dev_info *bdi = inode_to_bdi(inode);
//		unsigned long flags;
//
...
//				struct bdi_writeback *wb = inode_to_wb(inode);
//				wb_stat_mod(wb, WB_WRITEBACK, nr);
//				if (!on_wblist) wb_inode_writeback_start(wb);
//			}
//
//			if (mapping->host && !on_wblist) sb_mark_inode_writeback(mapping->host);
//			if (!folio_test_dirty(folio)) cc_xas_clear_mark(&xas, PAGECACHE_TAG_DIRTY);
//			if (!keep_write) cc_xas_clear_mark(&xas, PAGECACHE_TAG_TOWRITE);
//		}
...
//	return ret;
//}
//EXPORT_SYMBOL(__cc_folio_start_writeback);
